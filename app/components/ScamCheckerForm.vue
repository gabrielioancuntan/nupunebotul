<template>
  <section
    class="grid gap-3.5 rounded-lg border border-[#dce5dc] bg-white/75 p-[22px]"
    aria-label="Formular verificare mesaj"
  >
    <label class="font-extrabold text-[#162018]" for="scam-message"
      >Mesaj suspect</label
    >
    <textarea
      id="scam-message"
    ...
    <div class="border-t border-[#dce5dc] pt-4">
      <p class="mb-2.5 mt-0 font-bold text-[#536056]">
        Testează rapid cu un exemplu:
      </p>
...
const hasTrackedConversion = ref(false);
const message = ref("");
const error = ref("");
...
function analyzeMessage() {
  ...
  if (
    !hasTrackedConversion.value &&
    typeof window !== "undefined" &&
    typeof window.gtag === "function"
  ) {
    window.gtag("event", "conversion", {
      send_to: "AW-17831462931/A7MiCLHE3aocEJOQ2rZC",
    });
    hasTrackedConversion.value = true;
  }
...
function clearMessage() {
  message.value = "";
  error.value = "";
  hasTrackedConversion.value = false;
  emit("analyzed", null);
}
</script>
